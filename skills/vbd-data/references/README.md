...
Only Foundation labs that ship with sample data have a folder here. Labs with no seed data (warehouse — data is generated by T-SQL scripts at runtime; rti — uses Fabric built-in sample streams) intentionally have **no reference folder**.

| Folder | SharePoint source | Original dataset | Status |
|---|---|---|---|
| `lakehouse/` | `01 - Lakehouse Lab/Data` | Wide World Importers (retail) | ✓ `dimension_customer.csv` committed; full WWI zip (1.9 GB) fetched via `fetch-wwi-data.ps1` |
| `datascience/` | `04 - Data Science Lab/Data` | NY Taxi trips (regression target: fare / tip) | ✓ `ny-yellow-taxi-location-info.csv` (lookup) committed; main trips dataset fetched from `wasbs://` at lab time |
